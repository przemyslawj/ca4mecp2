...
%
% The field '.r1' is guaranteed to exist, and defines the rectangular
...
% This code is modified from version by Dylan Muir <dylan.muir@unibas.ch>.
...
switch nTypeID
   case 1
      ...
      [x,y] = meshgrid(sROI.vnRectBounds(1):sROI.vnRectBounds(3),sROI.vnRectBounds(2):sROI.vnRectBounds(4));
      sROI.mnCoordinates = [x(:) y(:)];
   ...
   case 2
      % - Oval
      sROI.strType = 'Oval';
      ra =(sROI.vnRectBounds(3) - sROI.vnRectBounds(1)) / 2;
      rb =(sROI.vnRectBounds(4) - sROI.vnRectBounds(2)) / 2;
      x0 = sROI.vnRectBounds(1) + ra;
      y0 = sROI.vnRectBounds(2) + rb;

      [x,y] = meshgrid(sROI.vnRectBounds(1):sROI.vnRectBounds(3),sROI.vnRectBounds(2):sROI.vnRectBounds(4));
      in_ellipse = (x - x0).^2 ./ ra.^2 + (y - y0).^ 2 / rb.^2 <= 1;
      [row,col] = find(in_ellipse == 1);
      sROI.mnCoordinates = [row col];
